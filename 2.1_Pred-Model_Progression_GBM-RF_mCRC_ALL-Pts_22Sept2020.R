...
all.aug$Progression <- factor(ifelse(all.aug$Progression==0,"Non_Progressed","Progressed"),
                              levels=c("Non_Progressed","Progressed"))
...
all.pfs <- dat.mv[,c(2,9:11,13,15:18,37,21:23,25,27,31,34)] # To predict Progression
...
#[1] 380.4   # This 1st imputed-patient have been predicted the 40% of times as "0" (= wild-type), and the 60% of times as "1" (= mutated)!
...
#[1] 99.24   # This 1st imputed-patient have been predicted the 40% of times as "0" (= wild-type), and the 60% of times as "1" (= mutated)!
...
#[1] 6901   # This 1st imputed-patient have been predicted the 40% of times as "0" (= wild-type), and the 60% of times as "1" (= mutated)!
...
#[1] 5.205   # This 1st imputed-patient have been predicted the 40% of times as "0" (= wild-type), and the 60% of times as "1" (= mutated)!
...
completeData.pcr
all.aug[rownames(completeData.pcr),"PCR"]


#
secure.imputed2 <- all.aug
secure2 <- all.aug

#setwd("~/Desktop/Analysis/Metastatic-mCRC_JMaurel_2020/Predictive-Model_ML/Checkpoint_files")
#write.table(all.aug,"Secure-Checkpoint_Pred-Model_mCRC_After-Imputation_All-Variables_22Sept2020_v3.txt",
...
all.aug$Progression <- factor(ifelse(all.aug$Progression==1,"Progressed","Non_Progressed"),
                              levels=c("Progressed","Non_Progressed"))
...
# (a) CLINICO-GENETIC [22nd September]!
...
# (c) GENETIC Model (v1) -- Adding KRAS, BRAF and PIK3CA:
#all <- cbind(folds, all.aug[,1:7])

# (d) GENETIC Model (v2) -- Without KRAS, BRAF and PIK3CA:
...
for(f in 1:length(number_folds))
{
  ...
  gbmFit1 <- train(Progression ~ ., data = train, 
...
pred.class <- data.frame(class=ifelse(prob.folds$Pred_class=="Progressed", "1", "0"))   # We can use the fitted prob to get) the prediction
rownames(pred.class) <- rownames(prob.folds)


## 6.2- REAL Classes (derived from Real-Observations):
real.class <- data.frame(class=all[order(match(rownames(all),rownames(pred.class))),"Progression"])
rownames(real.class) <- rownames(pred.class)
real.class$class <- ifelse(real.class$class=="Progressed","1","0")
...
### (B) GOOD!!! CLINICO-GENETIC MODEL [22nd September] --- RandomForest (adding KRAS, BRAF and PIK3CA):
diagnosticErrors(cm)
#acc      sens      spec       ppv       npv       lor 
#0.8150000 0.8181818 0.7692308 0.9807692 0.2272727 2.7080502 

### (B.2) GOOD!!! CLINICO-GENETIC MODEL [22nd September] --- RandomForest (without KRAS, BRAF and PIK3CA):
diagnosticErrors(cm)
#acc      sens      spec       ppv       npv       lor 
#0.8100000 0.8206522 0.6875000 0.9679487 0.2500000 2.3092296 

### (B.3) GOOD!!! CLINICAL MODEL [22nd September] --- RandomForest:
diagnosticErrors(cm)
#acc      sens      spec       ppv       npv       lor 
#0.8150000 0.8251366 0.7058824 0.9679487 0.2727273 2.4270127 

### (B.4) GOOD!!! GENETIC MODEL [22nd September] --- RandomForest (adding KRAS, BRAF and PIK3CA):
diagnosticErrors(cm)
#acc      sens      spec       ppv       npv       lor 
# 0.76500000  0.77948718  0.20000000  0.97435897  0.02272727 -0.12361396 

### (B.5) GOOD!!! GENETIC MODEL [22nd September] --- RandomForest (without KRAS, BRAF and PIK3CA):
diagnosticErrors(cm)
#acc      sens      spec       ppv       npv       lor 
#0.77500000 0.78172589 0.33333333 0.98717949 0.02272727 0.58260531 
...
prop.table(table(all.aug$Progression))["Progressed"]
#Progressed 
#0.78

# Apply BDtest function:
ci.values <- BDtest(ci.mat, pr=0.7727273, conf.level=0.90) # The prevalence is the REAL-proportion of Recurrent-patients!
...
#Sensitivity 0.8181818       0.7768345       0.7654155       0.8632029
#Specificity 0.7692308       0.5557366       0.5053503       0.9339504
ci.values$PPVNPVDAT # Estimations of PPV & NPV, & their 95% CI
#     Estimate Lower 90% limit Lower 95% limit Upper 95% limit
#NPV 0.5544354       0.4850939       0.4654247       0.6400863
#PPV 0.9233983       0.8628305       0.8395131       0.9652522


###### ###### ######
### (B) GOOD!!! CLINICO-GENETIC MODEL [22nd September] --- RandomForest (without KRAS, BRAF and PIK3CA):
ci.values$INDAT
#                 True positive True negative
#Test positive           151             5
#Test negative            33            11
ci.values$SESPDAT # Estimations of Sensitivity & Specificity, & their 95% CI
#             Estimate Lower 90% limit Lower 95% limit Upper 95% limit
#Sensitivity 0.8206522       0.7790962       0.7676190       0.8657466
#Specificity 0.6875000       0.4964901       0.4516529       0.8678890
ci.values$PPVNPVDAT 
#     Estimate Lower 90% limit Lower 95% limit Upper 95% limit
#NPV 0.5299539       0.4561512       0.4354365       0.6223706
#PPV 0.8992818       0.8470969       0.8287386       0.9427745


###### ###### ######
### (B) GOOD!!! CLINICAL MODEL [22nd September] --- RandomForest:
ci.values$INDAT
#                 True positive True negative
#Test positive           151             5
#Test negative            32            12
ci.values$SESPDAT # Estimations of Sensitivity & Specificity, & their 95% CI
#             Estimate Lower 90% limit Lower 95% limit Upper 95% limit
#Sensitivity 0.8251366       0.7837685       0.7723378       0.8698352
#Specificity 0.7058824       0.5219285       0.4780823       0.8762291
ci.values$PPVNPVDAT # Estima
#     Estimate Lower 90% limit Lower 95% limit Upper 95% limit
#NPV 0.5428119       0.4710971       0.4508549       0.6319411
#PPV 0.9051106       0.8546877       0.8368243       0.9466423



###### ###### ######
### (B) GOOD!!! GENETIC MODEL [22nd September] --- RandomForest (adding KRAS, BRAF and PIK3CA):
ci.values$INDAT
#                 True positive True negative
#Test positive           152             4
#Test negative            43             1
ci.values$SESPDAT # Estimations of Sensitivity & Specificity, & their 95% CI
#             Estimate Lower 90% limit Lower 95% limit Upper 95% limit
#Sensitivity 0.7794872      0.73679158      0.72504493       0.8274415
#Specificity 0.2000000      0.02085164      0.01020622       0.6574083
ci.values$PPVNPVDAT # Estimations of PPV & NPV, & their 95% CI
#     Estimate Lower 90% limit Lower 95% limit Upper 95% limit
#NPV 0.2105831      0.07723067      0.05682954       0.5414950
#PPV 0.7681332      0.71240819      0.69523352       0.8279116


###### ###### ######
### (B) GOOD!!! GENETIC MODEL [22nd September] --- RandomForest (without KRAS, BRAF and PIK3CA):
ci.values$INDAT
#                 True positive True negative
#Test positive           154             2
#Test negative            43             1
ci.values$SESPDAT # Estimations of Sensitivity & Specificity, & their 95% CI
#             Estimate Lower 90% limit Lower 95% limit Upper 95% limit
#Sensitivity 0.7817259      0.73939060      0.72773822       0.8292402
#Specificity 0.3333333      0.03451062      0.01695243       0.8646496
ci.values$PPVNPVDAT # Es
#     Estimate Lower 90% limit Lower 95% limit Upper 95% limit
#NPV 0.3099433       0.1345938       0.1032531       0.6366426
#PPV 0.7994707       0.7021545       0.6700987       0.8866879
...
#
prob.folds$Pred_prob_exit <- prob.folds$Progressed
pred.probs22 <- prob.folds$Progressed
...
# CORRECT AUC!!!!!!!!!!!
roc.obj1 <- roc(real.class3 ~ pred.probs22) # CLINICO-GENETIC Model [22nd September] -- Adding KRAS, BRAF and PIK3CA
roc.obj2 <- roc(real.class3 ~ pred.probs22) # CLINICO-GENETIC Model [22nd September] -- Without KRAS, BRAF and PIK3CA
roc.obj_B <- roc(real.class3 ~ pred.probs22) # CLINICAL Model [22nd September]
roc.obj_C1 <- roc(real.class3 ~ pred.probs22) # GENETIC Model [22nd September] -- Adding KRAS, BRAF and PIK3CA
roc.obj_C2 <- roc(real.class3 ~ pred.probs22) # GENETIC Model [22nd September] -- Without KRAS, BRAF and PIK3CA


### Time-dependent ROC/AUC-scores:
#library(survivalROC)
#survivalROC(Stime = all$PFS_days,
#            status       = dat.mv$Progression,
#            marker       = dat.mv$lp.clin,
#            predict.time = 365.25,  # Predict AUC on 1 time-point (at 1 year = 365 days)
#            method       = "KM")


### Derived-cutpoints of the ROC:
coords <- coords(roc.obj1, x="best", input="threshold") # OR "x="all""!
#  threshold specificity sensitivity
#1      0.79   0.8181818   0.6474359


### AUC Scores -- 6 Models:
auc.aug1 <- auc(real.class3 ~ pred.probs22) # RandomForest - CLINICO-GENETIC Model [22nd September] --- Adding KRAS, BRAF and PIK3CA
#Area under the curve: 0.7807
auc.aug2 <- auc(real.class3 ~ pred.probs22) # RandomForest - CLINICO-GENETIC Model [22nd September] --- Without KRAS, BRAF and PIK3CA
#Area under the curve: 0.7798
auc.aug_B <- auc(real.class3 ~ pred.probs22) # RandomForest - CLINICAL Model [22nd September]
#Area under the curve: 0.7821
auc.aug_C1 <- auc(real.class3 ~ pred.probs22) # RandomForest - GENETIC Model [22nd September] --- Adding KRAS, BRAF and PIK3CA
#Area under the curve: 0.5479
auc.aug_C2 <- auc(real.class3 ~ pred.probs22) # RandomForest - GENETIC Model [22nd September] --- Without KRAS, BRAF and PIK3CA
#Area under the curve: 


### 95% CI of the previous AUC-Scores (DeLong Test):
ci.auc(auc.aug1) # CLINICO-GENETIC Model --- Test DeLong for == 95% Confidence-Intervals (CIs) of AUC:
#95% CI: 0.7077-0.8537 (DeLong)
ci.auc(auc.aug2)
#95% CI: 0.7058-0.8537 (DeLong)
ci.auc(auc.aug_B) # CLINICAL Model [22nd September]
#95% CI: 0.7067-0.8576 (DeLong)
ci.auc(auc.aug_C1) # GENETIC Model [22nd September]
#95% CI: 0.4492-0.6466 (DeLong)
ci.auc(auc.aug_C2)
#95% CI: 0.4471-0.5921 (DeLong)


# Test DeLong to compare AUC derived from 2 Models:
...
#[1] 0.7829892
roc.test(roc.obj1,roc.obj_C1, method=c("delong"))$p.value # CLINICO-GENOMIC vs. GENETIC Models [21st September]
#[1] 0.001195796
roc.test(roc.obj_B,roc.obj_C1, method=c("delong"))$p.value # CLINICAL vs. GENETIC Models [21st September]
#[1] 0.0008547181
...
### (a) GOOD!!! --- Plot 3 ROC curves (in 1 graph):
...
### (b) GOOD!!! --- Plot 6 ROC curves (in 1 graph):
roc_A <- plot(roc.obj1, print.auc=TRUE,
              col="#00139C", print.auc.y = .4, print.auc.x = 0.2, xaxt="none")
axis(1, seq(0,1,0.25))
roc_A <- plot(roc.obj2, print.auc=TRUE,
              col="#0321FE", print.auc.y = .35, print.auc.x = 0.2, add = TRUE)
roc_A <- plot(roc.obj_B, print.auc = TRUE, 
              col = "#A1CB1D", print.auc.y = .3, print.auc.x = 0.2, add = TRUE)
roc_A <- plot(roc.obj_C1, print.auc = TRUE, 
              col = "#E88406", print.auc.y = .25, print.auc.x = 0.2, add = TRUE)
roc_A <- plot(roc.obj_C2, print.auc=TRUE,
              col="#D00808", print.auc.y = .2, print.auc.x = 0.2, add = TRUE)
...
ggroc(list(Clinico_Genomic = roc.obj1, Clinical = roc.obj_B, Genomic = roc.obj_C1)) +
...
###### ###### ######
### OTHER packages to draw ROC-curves:

### (a) GOOD/REGULAR!!! -- Receiver Operating Characteristics (ROC) Curve:
